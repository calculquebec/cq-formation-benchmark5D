...
int main(int argc, char* argv[])
{
    ...
    for (size_t cmID = 0; cmID < NB_CM; cmID++) {
        ...
        cm[cmID]->setBlockSize(cmargs.bsize1, cmargs.bsize2, cmargs.bsize3);
        cm[cmID]->setPrismSize(cmargs.psize1, cmargs.psize2, cmargs.psize3);
        cm[cmID]->setKSize(cmargs.ksize);
    }
...
CMArgs::CMArgs():
    algo(""), beta(false),
    n(0), ksize(32), n_runs(1),
    bsize1(16), bsize2(16), bsize3(16),
    psize1(8),  psize2(8),  psize3(8)
{
}
...
bool CMArgs::parse(int argc, char* argv[])
{
    ...
    while (++i < argc) {
        ...
        if (strcmp(argv[i], "-k") == 0 && (i + 1) < argc) ksize = atoi(argv[++i]);
        if (strcmp(argv[i], "-r") == 0 && (i + 1) < argc) n_runs = atoi(argv[++i]);

        if (strcmp(argv[i], "-b1") == 0 && (i + 1) < argc) bsize1 = atoi(argv[++i]);
        if (strcmp(argv[i], "-b2") == 0 && (i + 1) < argc) bsize2 = atoi(argv[++i]);
        if (strcmp(argv[i], "-b3") == 0 && (i + 1) < argc) bsize3 = atoi(argv[++i]);

        if (strcmp(argv[i], "-p1") == 0 && (i + 1) < argc) psize1 = atoi(argv[++i]);
        if (strcmp(argv[i], "-p2") == 0 && (i + 1) < argc) psize2 = atoi(argv[++i]);
        if (strcmp(argv[i], "-p3") == 0 && (i + 1) < argc) psize3 = atoi(argv[++i]);
    }
    ...
    if (ksize < 1 || n < ksize) {
        cerr << "Error: K must be inside range [" << 1 << ", " << n << "]" << endl;
        return false;
    }
    ...
    if (bsize1 > n || bsize2 > n || bsize3 > n) {
        cerr << "Error: -bX values must not exceed the value of N" << endl;
        return false;
    }

    if (psize1 > bsize1 || psize2 > bsize2 || psize3 > bsize3) {
        cerr << "Error: -pX values must not exceed the value of -bX" << endl;
        return false;
    }

    if (psize1 < 1 || psize2 < 1 || psize3 < 1) {
        cerr << "Error: -pX values must be at least 1" << endl;
        return false;
    }
...
void CMArgs::printArgs() const
{
    ...
    cout << "bsize: " << bsize1 << "," << bsize2 << "," << bsize3 << endl;
    cout << "psize: " << psize1 << "," << psize2 << "," << psize3 << endl;
...
void CMArgs::printCSV() const
{
    ...
    cout << bsize1 << "," << bsize2 << "," << bsize3 << ",";
    cout << psize1 << "," << psize2 << "," << psize3 << ",";
    cout << ksize << "," << n_runs;
}
...
void CMArgs::usage(char *cmd) const
{
    ...
    cout << "-bX sizeY   # Block size, component X, integer, pX <= bX <= N" << endl;
    cout << "-pX sizeY   # Prism size, component X, integer, 1 <= pX <= bX" << endl;
    cout << "-k ksize    # Groups of iterations along k-axis, 1 <= ksize <= N" << endl;
